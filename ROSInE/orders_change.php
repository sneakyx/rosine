...
switch ($_POST['next_function']) {
	case "insert_order":
			//insert an empty order- just to fill in next step
			...
			if (ucfirst(substr($_POST['contact_id'],0,1))=="P")
				...
				$customer_private=0;
			/* here we get the highest number of orders from the database
			 * this ist just for the use in the next functions
			 */
			//hier einfuegen 
			$_POST['order_id']=rosine_highest_number("order")+1;
			$query=rosine_correct_query("order", $rosine_db_query['insert_paperwork']);
			...
			if ($result!=false)
				...
				/* 
				 * although case insert_order ends here, after inserting empty order, 
...
//	break; //insert_order 
	...
	case "change":
		...
		$input_fields.='<input type="hidden" name="order_id" value="'.$_POST['order_id'].'"> ';
		...
		for ($i=0;$i<$items_per_page;$i++){
			...
			if ($_POST['articles'][$i]!="") {
				...
				if (substr($_POST['articles'][$i], 0,1)=="#"){
					$result=rosine_paperwork_add_article("order",$_POST['ammount'][$i],$_POST['order_id'],' ART_NUMBER = "'.substr($_POST['articles'][$i],1).'"');
				}// # is used to search directly for an article number
				else {
					$result=rosine_paperwork_add_article("order",$_POST['ammount'][$i],$_POST['order_id'],' ART_NAME  LIKE "%'.$_POST['articles'][$i].'%" OR ART_NUMBER LIKE "%'.$_POST['articles'][$i].'%"');
			...
			if ($select_article){
				// select fields must be displayed in order to select the right
		...
		for ($i=$items_per_page;$i<($items_per_page+$favorite_articles);$i++){
			...
			if ($_POST['ammount'][$i]>0){
				rosine_paperwork_add_article("order", $_POST['ammount'][$i], $_POST['order_id'],' ART_NUMBER = '.substr($_POST['articles'][$i],1));
		...
		if (mysql_errno($rosine_db)!=0) {
			...
			array_walk ($_POST[delete], 'rosine_delete_positions',str_replace("%paperwork%",
					"orders",$rosine_db_query['delete_article_from_paperwork']).
					' ORDER_ID= '.$_POST['order_id'].' AND POSI_ID=%value%');
			// is there anything to add from other tables?
			if ($_POST['offer']!=""){
				rosine_add_complete_paperwork("offer", $_POST['offer'], "order", $_POST['order_id']);
			}// if there is an offer to add
			// correct numbers
			rosine_correct_numbers("order", $_POST['order_id']);
			$liste=rosine_create_items_list("order", $_POST['order_id']);
			...
			// update order_ammount in table rosine orders
			rosine_update_ammount_paperwork("order", $_POST['order_id']);
		...
		if (mysql_errno($rosine_db)!=0) {
			...
			if (mysql_errno($rosine_db)!=0) {
				...
				$input_fields.='<form action="#" method="post">
							<input type="hidden" name="next_function" value="insert_order">';
...
$tpl->assign("paperwork", $lang['order']);
$tpl->assign("additional", '<h3>'.$lang['favorite_articles'].'</h3>
	{$input_favorites}
		<h3>'.$lang["add_articles_from_other_tables"].'</h3>
		{$offer_list}');
$tpl->assign("input_favorites", rosine_most_used_articles("order"));
$tpl->assign("offer_list", rosine_add_paperworklist("offer",$customer_details['contact_id']));
$tpl->assign("this_number",rosine_get_real_number("order",$_POST['order_id']));
$tpl->assign("paperwork_file", "orders");
$tpl->display();
