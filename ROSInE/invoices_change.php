...
// invoice list, add items etc
switch ($_POST['next_function']) {
	case "insert_invoice":
			//insert an empty invoice- just to fill in next step
			...
			if (ucfirst(substr($_POST['contact_id'],0,1))=="P")
				...
				$customer_private=0;
			/* here we get the highest number of invoices from the database
			 * this ist just for the use in the next functions
			 */

			$_POST['invoice_id']=rosine_highest_number("invoice")+1;
			$query=rosine_correct_query("invoice", $rosine_db_query['insert_paperwork']);
			$result=rosine_database_query($query.'('.
					$_POST['invoice_id'].', now(), '.
			...
			if ($result!=false)
				...
				/* 
				 * although case insert_invoice ends here, after inserting empty invoice, 
				 * this has to be filled with articles, so this break isn't used!
				 * 
				 */
//	break; //insert_invoice 
	...
	case "change":
		...
		$input_fields.='<input type="hidden" name="invoice_id" value="'.$_POST['invoice_id'].'"> ';
		...
		for ($i=0;$i<$items_per_page;$i++){
			...
			if ($_POST['articles'][$i]!="") {
				...
				if (substr($_POST['articles'][$i], 0,1)=="#"){
					$result=rosine_paperwork_add_article("invoice",$_POST['ammount'][$i],$_POST['invoice_id'],' ART_NUMBER = "'.substr($_POST['articles'][$i],1).'"');
				}// # is used to search directly for an article number
				else {
					$result=rosine_paperwork_add_article("invoice",$_POST['ammount'][$i],$_POST['invoice_id'],' ART_NAME  LIKE "%'.$_POST['articles'][$i].'%" OR ART_NUMBER LIKE "%'.$_POST['articles'][$i].'%"');
			...
			if ($select_article){
				// select fields must be displayed in invoice to select the right
		...
		for ($i=$items_per_page;$i<($items_per_page+$favorite_articles);$i++){
			...
			if ($_POST['ammount'][$i]>0){
				rosine_paperwork_add_article("invoice", $_POST['ammount'][$i], $_POST['invoice_id'],' ART_NUMBER = '.substr($_POST['articles'][$i],1));
		...
		if (mysql_errno($rosine_db)!=0) {
			...
			array_walk ($_POST[delete], 'rosine_delete_positions',str_replace("%paperwork%",
					"invoices",$rosine_db_query['delete_article_from_paperwork']).
					' INVOICE_ID= '.$_POST['invoice_id'].' AND POSI_ID=%value%');
			// is there anything to add from other tables?
			if ($_POST['offer']!=""){
				rosine_add_complete_paperwork("offer", $_POST['offer'], "invoice", $_POST['invoice_id']);
			}// if there is an offer to add
			if ($_POST['order']!=""){
				rosine_add_complete_paperwork("order", $_POST['order'], "invoice", $_POST['invoice_id']);
			}// if there is an order to add
			if ($_POST['delivery']!=""){
				rosine_add_complete_paperwork("delivery", $_POST['delivery'], "invoice", $_POST['invoice_id']);
			}// if there is an delivery to add
			
			// correct numbers
			rosine_correct_numbers("invoice", $_POST['invoice_id']);
			$liste=rosine_create_items_list("invoice", $_POST['invoice_id']);
			...
			// update invoice_ammount in table rosine invoices
			rosine_update_ammount_paperwork("invoice", $_POST['invoice_id']);
		...
		if (mysql_errno($rosine_db)!=0) {
			...
			if (mysql_errno($rosine_db)!=0) {
				...
				$input_fields.='<form action="#" method="post">
							<input type="hidden" name="next_function" value="insert_invoice">';
...
$tpl->assign("paperwork", $lang['invoice']);
...
$tpl->assign("input_favorites", rosine_most_used_articles("invoice"));
...
$tpl->assign("this_number",rosine_get_real_number("invoice",$_POST['invoice_id']));
$tpl->assign("paperwork_file", "invoices");
$tpl->display();
