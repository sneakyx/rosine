...
$tpl->load("deliverylist.html");
$lang[] = $language;
$lang = $tpl->loadLanguage($lang);
// should there be one delivery really deleted?
if ($_POST['next_function']=="really_delete"){
	$query=rosine_correct_query("delivery", $rosine_db_query['delete_paperwork'],"delivery");
	...
	if (mysql_errno($rosine_db)!=0) {
		...
		$OK.=str_replace("%paperwork%", $lang['delivery'], $lang['paperwork_deleted'])." ".$lang['']." ".$_POST['number']."<br>";
	}// no error while delete

	$query=rosine_correct_query("delivery", $rosine_db_query['delete_paperwork_positions'],"delivery");
	$result=mysql_query(str_replace("%ID%", $_POST['number'],$query));
	if (mysql_errno($rosine_db)!=0) {
		// Error in mysql detected
		...
		$OK.=str_replace("%paperwork%", $lang['delivery'], $lang['paperwork_positions_deleted'])." ".$lang['']." ".$_POST['number']."<br>";
	}// no error while delete
	
}// realley delete

// get ammount of deliveries in database
$result=mysql_query(str_replace("%paperwork%", "deliveries", $rosine_db_query['get_ammount_of_paperworks'])." 1");
$liste="";
if (mysql_errno($rosine_db)!=0) {
	// Error in mysql detected
...
if ($from < 0)
	...
	if ($max_rows<$from+$items_per_page)
		...
		$result=mysql_query(rosine_correct_query("delivery", $rosine_db_query['get_paperworks'],"deliveries")."1 ORDER BY DELIVERY_ID LIMIT $from,$items_per_page");
			...
			$tpl->assign("backward", '<a href="?from='.($from-$items_per_page).'">&lt;&lt;</a>');
				...
				<th>'.$lang['print'].'</th>		
							...
							while($f = @mysql_fetch_array($result)) {
								...
								if ($_POST['next_function']=="delete" & $_POST['number']==$f['DELIVERY_ID']){
									//Sicherheitsabfrage!
								...
								if ($f['delivery_customer_private']=="1")
									$f['n_fn'].=" ".$lang['private'];
								else 
									$f['n_fn'].=" ".$lang['company'];
								$liste.="<td>".$f['DELIVERY_ID']."</td>".
										"<td>".$f['DELIVERY_DATE']."</td>".
										"<td>".$f['n_fn']."</td>".
										"<td>".$f['DELIVERY_AMMOUNT']."</td>".
										"<td>".$f['DELIVERY_STATUS']."</td>".
										'<td>
					<form action="#" method="post">
						<input type="hidden" name="next_function" value="'.$next_function.'">
						<input type="submit" title="'.$delete.'" value="'.$delete.'">
						<input type="hidden" name="number" value="'.$f['DELIVERY_ID'].'">
					</form>
			</td>
			<td>
					<form action="deliveries_change.php" method="post">
						<input type="hidden" name="next_function" value="change">
						<input type="submit" title="'.$lang['change'].'" value="'.$lang['change'].'">
						<input type="hidden" name="delivery_id" value="'.$f['DELIVERY_ID'].'">
