...
$tpl->load("offerlist.html");
$lang[] = $language;
$lang = $tpl->loadLanguage($lang);
// should there be one order really deleted?
if ($_POST['next_function']=="really_delete"){
	$query=rosine_correct_query("offer", $rosine_db_query['delete_paperwork'],"offer");
	...
	if (mysql_errno($rosine_db)!=0) {
		...
		$OK.=str_replace("%paperwork%", $lang['offer'], $lang['paperwork_deleted'])." ".$lang['']." ".$_POST['number']."<br>";
	}// no error while delete

	$query=rosine_correct_query("offer", $rosine_db_query['delete_paperwork_positions'],"offer");
	$result=mysql_query(str_replace("%ID%", $_POST['number'],$query));
	if (mysql_errno($rosine_db)!=0) {
		// Error in mysql detected
		...
		$OK.=str_replace("%paperwork%", $lang['offer'], $lang['paperwork_positions_deleted'])." ".$lang['']." ".$_POST['number']."<br>";
	}// no error while delete
	
}// really delete

// get ammount of orders in database
$result=mysql_query(str_replace("%paperwork%", "offers", $rosine_db_query['get_ammount_of_paperworks'])." 1");
$liste="";
if (mysql_errno($rosine_db)!=0) {
	// Error in mysql detected
...
if ($from < 0)
	...
	if ($max_rows<$from+$items_per_page)
		...
		$result=mysql_query(rosine_correct_query("offer", $rosine_db_query['get_paperworks'],"offers")."1 ORDER BY OFFER_ID LIMIT $from,$items_per_page");
			...
			$tpl->assign("backward", '<a href="?from='.($from-$items_per_page).'">&lt;&lt;</a>');
				...
				<th>'.$lang['print'].'</th>		
							...
							while($f = @mysql_fetch_array($result)) {
								...
								if ($_POST['next_function']=="delete" & $_POST['number']==$f['OFFER_ID']){
									//Sicherheitsabfrage!
								...
								if ($f['offer_customer_private']=="1")
									$f['n_fn'].=" ".$lang['private'];
								else 
									$f['n_fn'].=" ".$lang['company'];
								$liste.="<td>".$f['OFFER_ID']."</td>".
										"<td>".$f['OFFER_DATE']."</td>".
										"<td>".$f['n_fn']."</td>".
										"<td>".$f['OFFER_AMMOUNT']."</td>".
										"<td>".$f['OFFER_STATUS']."</td>".
										'<td>
					<form action="#" method="post">
						<input type="hidden" name="next_function" value="'.$next_function.'">
						<input type="submit" title="'.$delete.'" value="'.$delete.'">
						<input type="hidden" name="number" value="'.$f['OFFER_ID'].'">
					</form>
			</td>
			<td>
					<form action="offers_change.php" method="post">
						<input type="hidden" name="next_function" value="change">
						<input type="submit" title="'.$lang['change'].'" value="'.$lang['change'].'">
						<input type="hidden" name="offer_id" value="'.$f['OFFER_ID'].'">
						<input type="hidden" name="contact_id" value="::'.$f['OFFER_CUSTOMER'].'">		
					</form>
			</td>'; //ob das mit den zwei Doppelpunkten stimmt?
								$liste.='<td><a href="print_paperwork.php?paperwork=offer&paperwork_id='.
										$f['OFFER_ID'].'" target="_blank">'.$lang['print'].'</a></td>';
								$liste.="</tr>";
							}
							$liste.="</table>";

						}
						$tpl->assign('offerlist', $liste);
						$tpl->assign("OK", $OK);
