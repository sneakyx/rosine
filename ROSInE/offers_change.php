...
switch ($_POST['next_function']) {
	case "insert_offer":
			...
			$query=rosine_correct_query("offer", $rosine_db_query['insert_paperwork']);
	...
	case "change":
		...
		for ($i=0;$i<($items_per_page);$i++){
			...
			if ($_POST['articles'][$i]!="") {
				...
				if (substr($_POST['articles'][$i], 0,1)=="#"){
					$result=rosine_paperwork_add_article("offer",$_POST['ammount'][$i],$_POST['offer_id'],' ART_NUMBER = "'.substr($_POST['articles'][$i],1).'"');
				}// # is used to search directly for an article number
				else {
					$result=rosine_paperwork_add_article("offer",$_POST['ammount'][$i],$_POST['offer_id'],' ART_NAME  LIKE "%'.$_POST['articles'][$i].'%" OR ART_NUMBER LIKE "%'.$_POST['articles'][$i].'%"');
			...
			$input_fields.='<input style="width:40px;" class="rosine_input_ammount" name="ammount['.$i.']" type="text" width="5" maxwidth="10" placeholder="'.
		...
		for ($i=$items_per_page;$i<($items_per_page+$favorite_articles);$i++){
			...
			if ($_POST['ammount'][$i]>0){
				rosine_paperwork_add_article("offer", $_POST['ammount'][$i], $_POST['offer_id'],' ART_NUMBER = '.substr($_POST['articles'][$i],1));
		...
		if (mysql_errno($rosine_db)!=0) {
			...
			array_walk ($_POST[delete], 'rosine_delete_positions',str_replace("%paperwork%",
					"offers",$rosine_db_query['delete_article_from_paperwork']).
					' OFFER_ID= '.$_POST['offer_id'].' AND POSI_ID=%value%');
			// funktion zum korrieren der Nummern?
			rosine_correct_numbers("offer", $_POST['offer_id']);
			$liste=rosine_create_items_list("offer", $_POST['offer_id']);
			...
			// update offer_ammount in table rosine offers
			rosine_update_ammount_paperwork("offer", $_POST['offer_id']);
...
$tpl->assign("paperwork_file", "offers");
$tpl->assign("additional", '<h3>'.$lang['favorite_articles'].'</h3>
	{$input_favorites}');

$tpl->assign("input_favorites", rosine_most_used_articles("offer"));
$tpl->assign("this_number",rosine_get_real_number("offer",$_POST['offer_id']));
$tpl->display();
