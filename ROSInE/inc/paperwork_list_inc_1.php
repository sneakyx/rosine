...
$result=rosine_database_query(rosine_correct_query($_POST['paperwork'], $rosine_db_query['get_paperworks'],
		$_POST['paperwork'])."1 ORDER BY ".strtoupper($_POST['paperwork'])."_ID DESC LIMIT $from,".
		$config['items_per_page'],120);

if ($from >0) {//show last if possible
...
if ($from < $max_rows-$config['items_per_page']){ //vorblaettern anzeigen wenn notwendig
	$tpl->assign("foreward", '<a href="?from='.($from+$config['items_per_page']).'&paperwork='.
			$_POST['paperwork'].'">&gt;&gt;</a>');
}//endif
else {
	$tpl->assign("foreward", "");
